...
public class Bibliotheque {
	...
	public void chargerLaBaseDeDonnéesEnMetier() {
		listeArtistes = DAOArtisteJPAFake.getInstance().loadAll();
		listeAlbums = DAOAlbumJPAFake.getInstance().loadAll();
		listeGenres = DAOGenreJPAFake.getInstance().loadAll();
		listeMorceaux = DAOMorceauJPAFake.getInstance().loadAll();
	}

	public void sauvegarderLaPartieMetierEnBaseDeDonnées() {
		DAOAlbumJPAFake.getInstance().saveAll(listeAlbums);
		DAOArtisteJPAFake.getInstance().saveAll(listeArtistes);
		DAOMorceauJPAFake.getInstance().saveAll(listeMorceaux);
	}
